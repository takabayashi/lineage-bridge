#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# LineageBridge UC Demo — Full Provisioning
#
# Two-step Tableflow API key bootstrap:
#   1. First terraform apply — creates environment, cluster, SA, AWS, Databricks
#   2. Creates a Tableflow API key via Confluent CLI
#   3. Second terraform apply — creates Tableflow topics + catalog integration
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
echo ""
echo "══════════════════════════════════════════════════════════════════"
echo "  LineageBridge UC Demo — Provisioning"
echo "  (Tableflow → Databricks Unity Catalog)"
echo "══════════════════════════════════════════════════════════════════"
echo ""
...
if [[ -n "$CACHE_ENV_ID" && -n "$CACHE_CLUSTER_ID" ]]; then
  ( cd "$PROJECT_DIR" && uv run python scripts/cache_demo_credentials.py \
      ...
      --demo-name uc ) || echo "  Warning: failed to update local credential cache"
fi

echo ""
echo "══════════════════════════════════════════════════════════════════"
echo "  UC Demo provisioned successfully!"
echo ""
echo "  Commands:"
