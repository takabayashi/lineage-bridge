...
Despite living under `phases/`, this is **not** an `ExtractionPhase` — it
runs post-extraction in `run_enrichment()`, not inside `PhaseRunner`. The
signature takes `(settings, graph)` directly because there is no
per-environment context at this stage; the graph already spans every
environment.

Iterates the distinct cluster IDs already on graph nodes; one Metrics API call
per cluster. Failures per cluster are logged and don't abort the rest.
"""
...
async def run_metrics_enrichment(
    settings: Settings,
    graph: LineageGraph,
    *,
